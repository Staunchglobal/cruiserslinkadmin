...
import {
  CategoriesToName,
  TipsAndTricksCategories,
  TipsAndTricksSubCategories,
} from "../util/constants";
...
const TipAndTrick = function (props) {
  ...
  const [totalPages, setTotalPages] = useState(
    Math.floor(stats.total / limit) + 1
  );
  const [activePage, setActivePage] = useState(1);
  const [activeState, setActiveState] = useState("all");
  useEffect(() => {
    if (activeState === "all") {
      setTotalPages(Math.floor(stats.total / limit) + 1);
      setActivePage(1);
    } else if (activeState === "pending") {
      setTotalPages(Math.floor(stats.pending / limit) + 1);
      setActivePage(1);
    } else if (activeState === "active") {
      setTotalPages(Math.floor(stats.active / limit) + 1);
      setActivePage(1);
    } else if (activeState === "inactive") {
      setTotalPages(Math.floor(stats.inactive / limit) + 1);
      setActivePage(1);
    }
  }, [activeState]);
  ...
  const fetchTipsAndTricks = function () {
    if (tipsandtricks.data.length > 0) {
      firestore
        ...
        .startAfter(
          tipsandtricks.data[tipsandtricks.data.length - 1].timePosted
        )
        .limit(limit)
        .get()
        .then((snapshot) => {
          setTipsAndTricks(intId, [
            ...tipsandtricks.data,
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          await Swal.fire({
            title: "Success",
            text: "Tip And Trick Deleted Successfully",
            icon: "success",
          });
          setTipsAndTricks(
            intId,
            tipsandtricks.data.filter((x) => x.id !== id)
          );
        } catch (err) {
          console.log(err);
  ...
  const EnableDisableTipAndTrick = function (id, value) {
    firestore
      ...
      .then((y) => {
        setTipsAndTricks(
          intId,
          tipsandtricks.data.map((x) =>
            x.id === id ? { ...x, status: value ? "active" : "inactive" } : x
          )
        );
        Swal.fire({
          title: "Success!",
          text: "Tip And Trick Changed Successfully",
          icon: "success",
        }).then((_) => {});
      })
      .catch((err) => {
  ...
  const data = tipsandtricks.data.filter((tip_and_trick, index) => {
    ...
    const typeFilter =
      activeState === "all" ? true : tip_and_trick.status === activeState;
    const pageFilter =
      index + 1 >= activePage - 1 * limit && index + 1 < activePage * limit;
  ...
  return data.length === 0 ? (
    <div>
      ...
      <div className="form-row">
        <div className="col">No Tips And Tricks Present Yet.</div>
      </div>
    </div>
  ) : (
    <div id="Yacht" className="id-div">
      <h5 className="text-primary">
        {TipsAndTricksCategories[intId]} (
        {activeState === "all"
          ? stats.total
          : activeState === "pending"
          ? stats.pending
                ...
                <table className="table table-bordered table-striped">
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>
                        {entry.SubCategory
                          ? TipsAndTricksSubCategories[entry.SubCategory]
                          : "--"}
                      </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                      <th>Ratings</th>
                      <td>
                        <Link to={`/ratings/tipandtrick/${entry.id}`}>
                          View Ratings
                        </Link>
                      </td>
                    </tr>
                    <tr>
                      ...
                      <td>
                        <Link
                          ...
                          to={`/edit_tips/${entry.category}/${entry.id}`}
                        >
                          Edit
                        </Link>
                        {entry.status === "pending" ? (
                          <Link
                            onClick={(e) => {
                              e.preventDefault();
                              EnableDisableTipAndTrick(entry.id, true);
                            }}
                            className="text-primary mr-3"
                            to="#"
                          >
                          ...
                          <Link
                            onClick={(e) => {
                              e.preventDefault();
                              EnableDisableTipAndTrick(entry.id, false);
                            }}
                        ...
                        <Link
                          onClick={(e) => handleDeleteTipAndTrick(e, entry.id)}
                          to="#"
                          className="text-danger mr-3"
                        >
                          Delete
                    ...
                    <tr>
                      ...
                      <td>
                        <Link
                          to={`/tiptrickshow/${entry.category}/${entry.id}`}
                        >
                          View Tip And Trick
                        </Link>
                      </td>
