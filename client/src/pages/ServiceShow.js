  ...
  useEffect(() => {
    ...
    if (!serviceData.reviewedByAdmin) {
      firestore
        .collection("Services")
        .doc(serviceData.id)
        .update({ reviewedByAdmin: true })
        .then((x) => {});
      setServices(
        intCat,
        services.data.map((x) =>
          x.id === serviceData.id ? { ...x, reviewedByAdmin: true } : x
        )
      );
    }
  }, []);
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          setServices(
            intCat,
            services.data.filter((x) => x.id !== id)
          );
          history.goBack();
        } catch (err) {
              ...
              <table className="table table-bordered table-striped">
                <tbody>
                  <tr>
                    <th
                      style={{
                        backgroundColor: serviceData.reviewedByAdmin
                          ? "green"
                          : "red",
                        color: "white",
                      }}
                  ...
                  <tr>
                    <th>Time</th>
                    <td>
                      {serviceData.StartTime}:00 - {serviceData.EndTime}:00
                    </td>
                  </tr>
                  <tr>
                    <th>Status</th>
                  ...
                  <tr>
                    <th>Ratings</th>
                    <td>
                      <Link to={`/ratings/service/${serviceData.id}`}>
                        View Ratings
                      </Link>
                    </td>
                  </tr>
                  ...
                  <tr>
                    ...
                    <td>
                      <Link
                        ...
                        to={`/edit_service/${serviceData.Category}/${serviceData.id}`}
                      >
                        Edit
                      </Link>
                      {serviceData.ServiceStatus === "pending" ? (
                        <Link
                          onClick={(e) => {
                            e.preventDefault();
                            EnableDisableService(serviceData.id, true);
                          }}
                          className="text-primary mr-3"
                          to="#"
                        >
                        ...
                        <Link
                          onClick={(e) => {
                            e.preventDefault();
                            EnableDisableService(serviceData.id, false);
                          }}
...
const mapStateToProps = (state, ownProps) => {
  ...
  return {
    ...
    serviceData: state.servicesReducer.services[intcat].data.find(
